...
-- best alpha
SELECT set_eq(
    $$SELECT st_area(geom)::TEXT FROM pgr_alphaShape((SELECT array_agg(geom) FROM s_test))$$,
    $$SELECT st_area(geom)::TEXT FROM pgr_alphaShape((SELECT array_agg(geom) FROM s_test), 0)$$
);
SELECT todo_end();

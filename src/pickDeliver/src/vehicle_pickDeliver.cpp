...
Vehicle_pickDeliver::pop_back() {
    ...
    ID deleted_pick = pick_itr->id();


    auto delivery_id = problem.nodes[deleted_pick].Did();
    ...
    pgassert(delivery_itr->Pid() == deleted_pick());
    ...
    ID deleted_order(0);
    //orders_in_vehicle.erase(orders_in_vehicle.find(deleted_order));

...
class Pgr_base_graph {
 public:
    ...
    private:
     ...
     void graph_add_edge(const boost_edge_t &edge);
     void graph_add_edge(const pgr_edge_t &edge);
};

template < class G >
void
Pgr_base_graph< G >::disconnect_edge(int64_t p_from, int64_t p_to) {
    V g_from;
    V g_to;
    boost_edge_t d_edge;
...
template < class G >
void
Pgr_base_graph< G >::disconnect_out_going_edge(int64_t vertex_id, int64_t edge_id) {
    Pgr_base_graph< G >::V v_from;
    boost_edge_t d_edge;
...
template < class G >
void
Pgr_base_graph< G >::disconnect_vertex(int64_t p_vertex) {
    V g_vertex;
    boost_edge_t d_edge;
    // nothing to do, the vertex doesnt exist
...
template < class G >
void
Pgr_base_graph< G >::restore_graph() {
...
template < class G >
bool
Pgr_base_graph< G >::get_gVertex(int64_t vertex_id, V &gVertex) const {
    Pgr_base_graph< G >::LI vertex_ptr = vertices_map.find(vertex_id);
...
template < class G >
int64_t
Pgr_base_graph< G >::get_edge_id(V from, V to, float8 &distance) const {
    E e;
    EO_i out_i, out_end;
    V v_source, v_target;
...
template < class G >
void
Pgr_base_graph< G >::adjust_vertices() {
...
template < class G >
void
Pgr_base_graph< G >::graph_add_edge(const boost_edge_t &edge ) {
    bool inserted;
    typename Pgr_base_graph< G >::LI vm_s, vm_t;
    typename Pgr_base_graph< G >::E e;
...
template < class G>
void
Pgr_base_graph< G >::graph_add_edge(const pgr_edge_t &edge) {
    bool inserted;
    typename Pgr_base_graph< G >::LI vm_s, vm_t;
    typename Pgr_base_graph< G >::E e;

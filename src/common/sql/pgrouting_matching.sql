...
BEGIN
    ...
    EXECUTE 'select geometryType(ST_GeometryFromText(''' || astext(line) ||
        ''', ' || srid || ')) as type' INTO row;
    ...
    LOOP
        ...
        EXECUTE 'select * from pgr_findNearestNodeDwithin(''POINT(' ||
            ST_X(ST_PointN(line, i)) || ' ' || ST_Y(PointN(line, i)) ||
        ...
        ELSE 
            ...
            EXECUTE 'select * from pgr_findNodeByNearestLinkDwithin(''POINT('
                        ||ST_X(ST_PointN(line, i))||' '||ST_Y(PointN(line, i))||')'','||distance2||', '''||tbl||''') as id' INTO row;
...
BEGIN
    ...
    EXECUTE 'select ST_LineMerge(ST_Union(ST_Multi(the_geom))) as the_geom ' ||
        ' from ' || tbl || ' where gid in (' || array_to_string(edges, ', ') ||
        ') and gid > 0' INTO row;

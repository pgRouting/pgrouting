...
/***********************************************************************
 *
 *   pgr_edgeBetweennessCentrality( edges_sql TEXT );
 *
 ***********************************************************************/
 ...
 *
 * @returns edges_centralities, when edges_centralities are found
 */

template < class G >
std::vector <EBC_rt>
pgr_edgeBetweennessCentrality(G &graph) {
    pgrouting::centrality::PgrCentralityGraph < G > fn_edgeBetweennessCentrality;
    auto edges_centralities = fn_edgeBetweennessCentrality.edgeBetweennessCentrality(graph);
    return edges_centralities;
}

/** @brief Performs exception handling and converts the edges_centralities to postgres.
 *
 ...
 * `pgr_edgeBetweennessCentrality` which calls the main function
...
do_pgr_edgeBetweennessCentrality(
    ...
    try {
        ...
        if (directed) {
            ...
            edges_centralities = G.get_edges_centralities();
        } else {
            pgrouting::centrality::PgrCentralityGraph<pgrouting::UndirectedGraph> G(data_edges, total_edges);
            edges_centralities = G.get_edges_centralities();
        }

        auto count = edges_centralities.size();

        if (count == 0) {
            (*return_tuples) = NULL;
            (*return_count) = 0;
            notice << "No edges_centralities found";
            *log_msg = pgr_msg(notice.str().c_str());
            return;
        }

        (*return_tuples) = pgr_alloc(edges_centralities.size(), (*return_tuples));
        for (size_t i = 0; i < edges_centralities.size(); i++) {
            (*return_tuples)[i] = edges_centralities[i];
        }
        *return_count = edges_centralities.size();

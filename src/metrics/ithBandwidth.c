...
PGDLLEXPORT Datum _pgr_ithBandwidth(PG_FUNCTION_ARGS);
...
process(
    ...
    time_msg("processing pgr_ithBandwidth", start_t, clock());
...
PG_FUNCTION_INFO_V1(_pgr_ithBandwidth);
PGDLLEXPORT Datum _pgr_ithBandwidth(PG_FUNCTION_ARGS) {
    ...
    if (funcctx->call_cntr < funcctx->max_calls) {
        ...
        values[1] = Float8GetDatum(result_tuples[funcctx->call_cntr].d2.value);

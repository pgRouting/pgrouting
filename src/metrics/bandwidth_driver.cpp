...
pgr_bandwidth(
    ...
    try {
        ...
        std::vector<pgrouting::Bandwidth_result> results;

        if (directed) {
            log << "Processing Directed graph\n";
            pgrouting::DirectedGraph digraph;
            digraph.insert_edges(edges);
            results = pgrouting::bandwidth(digraph);
        } else {
            log << "Processing Undirected graph\n";
            pgrouting::UndirectedGraph undigraph;
            undigraph.insert_edges(edges);
            results = pgrouting::bandwidth(undigraph);
        }
        ...
        *return_tuples = pgr_alloc(*return_count, (*return_tuples));
        ...
        for (size_t i = 0; i < *return_count; ++i) {
            (*return_tuples)[i].id = results[i].id;
            (*return_tuples)[i].bandwidth = results[i].bandwidth;
        }

...
#include "metrics/pgr_bandwidth.hpp"

/** @file bandwidth_driver.cpp
 * @brief Handles actual calling of function in the `pgr_bandwidth.hpp` file.
 *
 */

/***********************************************************************
 *
 *   pgr_bandwidth( edges_sql TEXT );
 *
 ***********************************************************************/
 ...
 * defined in the C++ Header file - `pgr_bandwidth.hpp`
 *
 * @param graph      the graph containing the edges
 * @param log        stores the log message
 *
 * @returns bandwidth, when bandwidth is found
 */

template < class G >
I_rt
pgr_bandwidth(G &graph) {
    pgrouting::bandwidth::PgrBandwidthGraph < G > fn_bandwidth;
    auto bandwidth = fn_bandwidth.bandwidth(graph);
    return bandwidth;
}

/** @brief Performs exception handling and converts the bandwidth to postgres.
 *
 ...
 * `pgr_bandwidth` which calls the main function
...
do_pgr_bandwidth(
    ...
    try {
        ...
        if (count == 0) {
            (*return_tuples) = NULL;
            (*return_count) = 0;
            notice << "No bandwidth found";
            *log_msg = pgr_msg(notice.str().c_str());
            return;
        }

        (*return_tuples) = pgr_alloc(1, (*return_tuples));
        (*return_tuples)[0] = bandwidth;

...
process( char* edges_sql,
  ...
  if (total_tuples == 0 || size_end_vidsArr == 0) {
    PGR_DBG("No edges found or no destinations found");
    (*result_count) = 0;
    (*result_tuples) = NULL;
    pgr_SPI_finish();
    return;
...
one_to_many_dijkstra_sql(PG_FUNCTION_ARGS) {
  ...
  /*******************************************************************************/
  /*                          MODIFY AS NEEDED                                   */
      // CREATE OR REPLACE FUNCTION pgr_dijkstra(sql text, start_vid bigint, end_vids_sql TEXT, directed boolean default true,

...
template < class G >
void
pgr_drivingDistance(G &graph, std::deque< Path > &paths,
...
template < class G >
void
pgr_drivingDistance(G &graph, Path &path,  int64_t  source, double distance) {
    Pgr_dijkstra< G > fn_dijkstra;
...
class Pgr_dijkstra {
    ...
    public:
        ...
        void
            drivingDistance(G &graph, Path &path, int64_t start_vertex, float8 distance) {
                clear();
                ...
                // perform the algorithm
                if ( dijkstra_1_to_distance(graph.graph, v_source, distance) ) {
                    // get the results
        ...
        void
            drivingDistance(G &graph, std::deque< Path > &paths,

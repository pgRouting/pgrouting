...
PREPARE v3e2q11 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size 
FROM ( VALUES (1, 1, 'v', -1, -1, -1, ARRAY[2, 3]::bigint[], 2) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size) ;
...
PREPARE v3e2q41 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 6, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1), (2, 7, 'v', -1, -1, -1, ARRAY[5]::integer[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v3e2q61 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 1, 'v', -1, -1, -1, ARRAY[2]::bigint[], 1), (2, 3, 'v', -1, -1, -1, ARRAY[4]::integer[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v3e2q71 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 3, 'v', -1, -1, -1, ARRAY[4]::bigint[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v4e3q21 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (-1, -1, 'v', -1, -1, -1, ARRAY[]::bigint[], 0) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size) WHERE 1 != 1 ;
...
PREPARE v4e3q51 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 6, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1), (2, 7, 'v', -1, -1, -1, ARRAY[5]::integer[], 1), (3, 8, 'v', -1, -1, -1, ARRAY[5]::integer[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v5e3q11 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 1, 'v', -1, -1, -1, ARRAY[2, 3]::bigint[], 2), (2, 6, 'v', -1, -1, -1, ARRAY[5]::integer[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v5e3q31 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 6, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v5e3q41 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 2, 'v', -1, -1, -1, ARRAY[3]::bigint[], 1), (2, 6, 'v', -1, -1, -1, ARRAY[5]::integer[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v6e3q11 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 1, 'v', -1, -1, -1, ARRAY[2]::bigint[], 1), (2, 3, 'v', -1, -1, -1, ARRAY[4]::integer[], 1), (3, 6, 'v', -1, -1, -1, ARRAY[5]::integer[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v6e3q21 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 1, 'v', -1, -1, -1, ARRAY[2]::bigint[], 1), (2, 3, 'v', -1, -1, -1, ARRAY[4]::bigint[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v6e3q31 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 1, 'v', -1, -1, -1, ARRAY[2]::bigint[], 1), (2, 6, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v4e4q11 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 7, 'v', -1, -1, -1, ARRAY[5, 6]::bigint[], 2), (2, 8, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);
...
PREPARE v4e4q31 AS
SELECT seq, id, type, source, target, cost, unnest(contracted_vertices) AS contracted_vertices, contracted_vertices_size FROM ( VALUES (1, 6, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1), (2, 7, 'v', -1, -1, -1, ARRAY[5]::bigint[], 1), (3, 8, 'v', -1, -1, -1,  ARRAY[5]::bigint[], 1) ) AS t(seq, id, type, source, target, cost, contracted_vertices, contracted_vertices_size);

...
private:
	...
	std::priority_queue<pq_pair,std::vector<pq_pair>,Compare> m_pq; // to store depot to order cost in decreasing order
	int m_iVehicleUsed;
	int m_iOrdersServed;
	int m_iTotalOrders;
	double m_dTotalCost;
...
private:
	...
	std::map<std::pair<int, int>, CostPack> m_mapOrderToDepotCost;
        ...
        std::priority_queue<pq_pair,std::vector<pq_pair>,Compare> do_pq; // to store depot to order cost in decreaing order

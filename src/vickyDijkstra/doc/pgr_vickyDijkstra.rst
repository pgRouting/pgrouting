...
``pgr_vickyDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: vickyDijkstra(Minimal Use)
...
.. code-block:: none
    ...
    pgr_vickyDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_vickyDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: vickyDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_vickyDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET

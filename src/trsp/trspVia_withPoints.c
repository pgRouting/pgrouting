...
PGDLLEXPORT Datum _pgr_trspvia_withpoints_v5(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(_pgr_trspvia_withpoints_v5);
...
PGDLLEXPORT Datum
_pgr_trspvia_withpoints_v5(PG_FUNCTION_ARGS) {
...
/* The 4.0 generation of pgr_trspVia_withPoints.
 * Kept writing the original 10 columns, without pred, so that a database
 * whose extension is still on 4.0.x keeps getting the shape its own SQL
 * declares. The 4.1 SQL binds to _pgr_trspvia_withpoints_v5 instead.
 */
PGDLLEXPORT Datum

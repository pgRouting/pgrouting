...
FROM pgr_apspWarshall('select id, source, target, cost from edge_table', false, false);

PREPARE q2 AS
SELECT  id1, id2, round(cost::numeric, 2) as cost
FROM pgr_apspWarshall('select id, source, target, cost from edge_table', TRUE, false);

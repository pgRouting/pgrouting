	       ...
	       dist_fromto_t **dists, char **err_msg)
{
		...
		*dists = (dist_fromto_t *) malloc(sizeof(dist_fromto_t) * nb_targets + 1);
		...
		for (int numTarget = 0; numTarget < nb_targets; numTarget++)
		{
			...
			(*dists)[numTarget].cost = 0;
			if (no_path[numTarget]){
				(*dists)[numTarget].cost = -1;
			}
			else{
				for(i = path_vect[numTarget].size() - 1, j = index_of_last_path_vertex; i >= 0; i--, j++)
				{
					...
					for (tie(out_i, out_end) = out_edges(v_src, graph); out_i != out_end; ++out_i)
					{
						...
						if (targ == v_targ)
						{
							if ((*dists)[numTarget].edge_id_source < 0) (*dists)[numTarget].edge_id_source = graph[*out_i].id;
...
int 
onetomany_dijkstra_boostpath(edge_t *edges, unsigned int count, int start_vertex, int *end_vertices, int nb_targets,
	       bool directed, bool has_reverse_cost,
			...
			if (no_path[numTarget]){
				...
				for(i = path_vect[numTarget].size() - 1, j = index_of_last_path_vertex; i >= 0; i--, j++)
				{
					...
					for (tie(out_i, out_end) = out_edges(v_src, graph); out_i != out_end; ++out_i)
					{
						...
						if (targ == v_targ)
						{
							if ((*pathdists)[numTarget].edge_id_source < 0) (*pathdists)[numTarget].edge_id_source = graph[*out_i].id;
							else edgeIDLists[numTarget] += ", ";
					//		(*pathdists)[numTarget].edge_id_source < 0 ? (*pathdists)[numTarget].edge_id_source = graph[*out_i].id : edgeIDLists[numTarget] += ", ";

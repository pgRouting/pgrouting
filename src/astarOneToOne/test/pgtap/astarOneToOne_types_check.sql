...
SELECT has_function('pgr_astaronetoone', ARRAY['text','bigint','anyarray','boolean']);
SELECT function_returns('pgr_astaronetoone', ARRAY['text','bigint','anyarray','boolean'],'setof record');
...
FROM ( 
    SELECT * FROM pgr_astaronetoone(
        'SELECT id::INTEGER, source::INTEGER, target::INTEGER, cost::FLOAT, reverse_cost::FLOAT FROM edge_table',
        2, ARRAY[3], true) ) AS a
limit 1;

SELECT set_eq('v21q01', 'v21q10','pgr_astaronetoone 1 to many: SHOULD RETURN expected columns names & types');
...
SELECT throws_ok('
    SELECT * FROM pgr_astaronetoone(
        ''SELECT id, source, target, cost, reverse_cost FROM edge_table'',
        2::FLOAT, ARRAY[5,3]);',
    '42883', 'function pgr_astaronetoone(unknown, double precision, integer[]) does not exist',
...
SELECT throws_ok('
    SELECT * FROM pgr_astaronetoone(
        ''SELECT id, source, target, cost, reverse_cost FROM edge_table'',
        2::REAL, ARRAY[5,3]);',
    '42883', 'function pgr_astaronetoone(unknown, real, integer[]) does not exist',

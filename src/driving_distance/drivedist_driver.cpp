...
do_pgr_driving_many_to_dist(
    ...
    try {
        ...
        if (directedFlag) {
            ...
            paths = pgrouting::drivingDistance(digraph, start_vertices, distance, equiCostFlag);
        } else {
            pgrouting::UndirectedGraph undigraph(vertices, gType);
            undigraph.insert_edges(data_edges, total_edges, true);
            paths = pgrouting::drivingDistance(undigraph, start_vertices, distance, equiCostFlag);
        }

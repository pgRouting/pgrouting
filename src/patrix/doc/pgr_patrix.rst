...
``pgr_patrix`` — [medium description of
...
.. code-block:: none
    ...
    pgr_patrix(edges_sql, start_vid,  end_vids)
    pgr_patrix(edges_sql, start_vids, end_vids, directed:=true)
...
.. index:: 
    single: pgr_patrix(edges_sql, start_vid,  end_vids) - proposed
...
.. literalinclude:: doc-patrix.queries
   :start-after: --q1
   :end-before: --q2
...
.. index:: 
    single: pgr_patrix(edges_sql, start_vids, end_vids, directed:=true) - proposed

.. code-block:: none

    pgr_patrix(  [parameters],

...
BEGIN;
   ...
   (SELECT path_seq, 1 AS end_pid, node, agg_cost FROM pgr_withPoints(
    ...
    SELECT seq, path_seq, -end_pid AS end_pid, node, agg_cost FROM pgr_withPoints(
        'SELECT id, source, target, cost, reverse_cost FROM edge_table ORDER BY id',
        '(SELECT pid, edge_id, fraction, side from points) UNION ALL (SELECT pid, edge_id, fraction, side from points)',
        3, ARRAY[5, 1, 1, 5, 5, 1],
        driving_side := 'r',
        directed := true,
        details := true);
    ...
    SELECT seq, path_seq, -end_pid AS end_pid, node, agg_cost FROM pgr_withPoints(
        'SELECT id, source, target, cost, reverse_cost FROM edge_table ORDER BY id',
        '(SELECT pid, edge_id, fraction, side from points) UNION ALL (SELECT 1 as pid, edge_id, fraction, side from points)',
        3, ARRAY[5, 1, 1, 5, 5, 1],
        driving_side := 'r',
        directed := true,
        details := true);

...
``pgr_pushRelabel`` — Calculates the maximum flow in a directed graph given a source and a destination. Implemented by Boost Graph Library.
...
.. index:: 
    single: pgr_pushRelabel(edges_sql, source,  sink)
...
.. code-block:: none
    ...
    pgr_funnyDijkstra(edges_sql, start_vid,  end_vids)
    RETURNS SET OF (seq, path_seq [, start_vid] [, end_vid], node, edge, cost, agg_cost)
      OR EMPTY SET

:Example:

.. literalinclude:: doc-funnyDijkstra.queries
   :start-after: --q1
   :end-before: --q2
...
.. index:: 
    single: pgr_funnyDijkstra(edges_sql, start_vids, end_vids, directed:=true) - proposed

.. code-block:: none

    pgr_funnyDijkstra(  [parameters],

...
#include "nonconst_breadth_first_search.hpp"
...
template<class AStarHeuristic, class UniformCostVisitor, class UpdatableQueue,
...
struct astar_bfs_visitor {
  ...
  astar_bfs_visitor(AStarHeuristic h, UniformCostVisitor vis, UpdatableQueue& Q,
      PredecessorMap p, CostMap c, DistanceMap d, WeightMap w, ColorMap col,
      BinaryFunction combine, BinaryPredicate compare, C zero) :
      m_h(h), m_vis(vis), m_Q(Q), m_predecessor(p), m_cost(c), m_distance(d), m_weight(
          w), m_color(col), m_combine(combine), m_compare(compare), m_zero(zero) {
  }
  ...
  void tree_edge(Edge e, const Graph& g) {
    ...
    if (m_decreased) {
      ...
      put(m_cost, target(e, g),
          m_combine(get(m_distance, target(e, g)), m_h(target(e, g))));
    } else
...
inline void astar_search_no_init(VertexListGraph &g,
  ...
  detail::astar_bfs_visitor<AStarHeuristic, AStarVisitor, MutableQueue,
      ...
      CombineFunction, CompareFunction> bfs_vis(h, vis, Q, predecessor, cost,
      distance, weight, color, combine, compare, zero);
...
template<typename VertexListGraph, typename AStarHeuristic,
    ...
    typename CostInf, typename CostZero>
inline void astar_search(VertexListGraph &g,
    typename graph_traits<VertexListGraph>::vertex_descriptor s,
    AStarHeuristic h, AStarVisitor vis, PredecessorMap predecessor,
  ...
  astar_search_no_init(g, s, h, vis, predecessor, cost, distance, weight, color,
...
void astar_search(VertexListGraph &g,
    typename graph_traits<VertexListGraph>::vertex_descriptor s,
  ...
  typedef typename detail::override_const_property_result<arg_pack_type,
  ...
  typedef typename detail::map_maker<VertexListGraph, arg_pack_type,
      tag::distance_map, W>::map_type distance_map_type;
  ...
  astar_search(
      ...
      detail::make_property_map_from_arg_pack_gen<tag::rank_map, D>(D())(g,
          arg_pack)
      ,
      detail::make_property_map_from_arg_pack_gen<tag::distance_map, W>(W())(g,
          arg_pack)
      ,
      detail::override_const_property(arg_pack, _weight_map, g, edge_weight)
      ,
      detail::override_const_property(arg_pack, _vertex_index_map, g,
          vertex_index),
...
template<typename VertexListGraph, typename AStarHeuristic, typename P,
    typename T, typename R>
void astar_search_no_init(VertexListGraph &g,
    typename graph_traits<VertexListGraph>::vertex_descriptor s,
    AStarHeuristic h, const bgl_named_params<P, T, R>& params) {
  using namespace boost::graph::keywords;
  typedef bgl_named_params<P, T, R> params_type;
  ...
  astar_search_no_init(
      ...
      detail::make_property_map_from_arg_pack_gen<tag::rank_map, D>(D())(g,
          arg_pack)
      ,
      detail::make_property_map_from_arg_pack_gen<tag::distance_map, D>(D())(g,
          arg_pack)
      ,
      detail::override_const_property(arg_pack, _weight_map, g, edge_weight)
      ,

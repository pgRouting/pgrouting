...
Datum shortest_path_mild_two_q(PG_FUNCTION_ARGS);
...
static int compute_shortest_path_mild_two_q(char* sql, int start_vertex, 
                                 ...
                                 path_element_t **path, int *path_count) 
{
  ...
  DBG("start shortest_path_two_q\n");
        
  SPIcode = SPI_connect();
  if (SPIcode  != SPI_OK_CONNECT)
    {
      elog(ERROR, "shortest_path_two_q: couldn't open a connection to SPI");
      return -1;
    }
  ...
  if ((SPIportal = SPI_cursor_open(NULL, SPIplan, NULL, NULL, true)) == NULL) 
    {
      elog(ERROR, "shortest_path_two_q: SPI_cursor_open('%s') returns NULL", sql);
      return -1;
    }
  ...
  DBG("Calling boost_two_q\n");
        
  start_vertex -= v_min_id;
  end_vertex   -= v_min_id;

  ret = boost_mild_two_q(edges, total_tuples, start_vertex, end_vertex,
...
PG_FUNCTION_INFO_V1(shortest_path_mild_two_q);
Datum
shortest_path_mild_two_q(PG_FUNCTION_ARGS)
{
  ...
  if (SRF_IS_FIRSTCALL())
    {
      ...
      ret = compute_shortest_path_mild_two_q(text2char(PG_GETARG_TEXT_P(0)),
                                  PG_GETARG_INT32(1),
                                  PG_GETARG_INT32(2),
                                  PG_GETARG_BOOL(3),

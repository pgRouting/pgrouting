/*PGR-GNU*****************************************************************
File: pgr_centralPointDominance.hpp
...
#ifndef INCLUDE_METRICS_PGR_CENTRALPOINTDOMINANCE_HPP_
#define INCLUDE_METRICS_PGR_CENTRALPOINTDOMINANCE_HPP_
#pragma once
...
class PgrCentralityGraph {
 public:
  ...
  get_central_point_dominance() {
    ...
    //Calculating Highest Relative Betweenness Centrality
    double highest_rbc = 0;
    for (const auto& vertex : vertices_centralities) {
        if (vertex.relative_bc > highest_rbc) {
            highest_rbc = vertex.relative_bc;
        }
    }
    ...
    for (const auto& vertex : vertices_centralities) {
        size_t n = boost::num_vertices(boost_graph) - 1;
        double temp = boost::is_directed(boost_graph)
            ? (1 / (double(n) - 1)) * (highest_rbc - (vertex.relative_bc / (n * n - 3 * n + 2)))
            : (1 / (double(n) - 1)) * (highest_rbc - ((vertex.relative_bc * 2) / (n * n - 3 * n + 2)));
        central_pd += temp;
    }

    central_point_dominance.value = central_pd;
...
#endif  // INCLUDE_METRICS_PGR_CENTRALPOINTDOMINANCE_HPP

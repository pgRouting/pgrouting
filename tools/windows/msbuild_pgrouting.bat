...
rem ### CGAL ###
rem TODO:better rebuild
rem if exist %CGAL_BUILD_DIR%\ if %REBUILD%==1 (
rem     rmdir /S /Q %CGAL_BUILD_DIR%
rem )
rem if not exist %CGAL_BUILD_DIR%\ (
rem     mkdir %CGAL_BUILD_DIR%
rem     pushd %CGAL_BUILD_DIR%
rem     @echo on
rem     cmake -G "%CMAKE_GENERATOR%" -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX=%COMMON_INSTALL_DIR% ^
rem         -DBoost_USE_MULTITHREADED=ON -DCGAL_Boost_USE_STATIC_LIBS=ON -DBoost_USE_STATIC_RUNTIME=OFF ^
rem         -DBoost_INCLUDE_DIR:PATH=%BOOST_INCLUDE_DIR% ^
rem         -DBOOST_LIBRARYDIR=%BOOST_LIBRARY_DIR% -DGMP_INCLUDE_DIR=%GMP_DIR%\include ^
rem         -DGMP_LIBRARIES=%GMP_DIR%\lib\%GMP_LIB_NAME% -DMPFR_INCLUDE_DIR=%GMP_DIR%\include ^
rem         -DMPFR_LIBRARIES=%GMP_DIR%\lib\%MPFR_LIB_NAME% -DWITH_CGAL_QT3=OFF -DWITH_CGAL_QT4=OFF ..\..\..\
rem     msbuild CGAL.sln /target:Build /property:Configuration=%MSBUILD_CONFIGURATION%
rem     msbuild INSTALL.%PROJ_EXT% /target:Build /property:Configuration=%MSBUILD_CONFIGURATION%
rem     @echo off
rem     popd
rem )

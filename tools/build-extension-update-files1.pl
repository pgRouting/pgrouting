...
sub generate_upgrade_script {
    ...
    die "ERROR: can not upgrade from $o_ver to n_ver" unless $new_version !~ $o_ver;
    ...
    write_script($o_ver, $new_version, \@types2remove, join('', @commands));
}


# we have all the data we need, so write the extension update script
sub write_script {
    my ($o_ver, $new_version, $types, $cmds) = @_;

    # open the extension update script or die if we can't
    open(OUT, ">$output_directory/pgrouting--$o_ver--$new_version.sql")
        || die "ERROR: failed to create '$output_directory/pgrouting-pgrouting--$o_ver--$new_version.sql' : $!\n";

    # write out the header and the commands to clean up the old extension
    print OUT <<EOF;
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
-- pgRouting extension upgrade from $o_ver to $new_version
...
EOF
    ...
    print OUT @file;
    close(OUT);
    print "  -- Created lib/pgrouting--$o_ver--$new_version.sql\n";
}

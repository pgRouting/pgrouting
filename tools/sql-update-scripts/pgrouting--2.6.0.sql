...
$BODY$
    SELECT '2.6.0'::varchar AS version,
        'v2.6.0-dev'::varchar AS tag,
        ''::varchar AS hash,
        ''::varchar AS branch,
...
'$libdir/libpgrouting-2.6', 'many_to_many_dijkstra'
LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_dijkstraVia(
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'dijkstraVia'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_johnson(edges_sql TEXT, directed BOOLEAN DEFAULT TRUE,
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'johnson'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_floydWarshall(edges_sql TEXT, directed BOOLEAN DEFAULT TRUE,
  OUT start_vid BIGINT, OUT end_vid BIGINT, OUT agg_cost float)
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'floydWarshall'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION _pgr_astar(
    ...
    OUT agg_cost FLOAT)
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'astarManyToMany'
LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_withPointsDD(
    ...
    OUT agg_cost FLOAT)
  RETURNS SETOF RECORD AS
     '$libdir/libpgrouting-2.6', 'many_withPointsDD'
...
CREATE OR REPLACE FUNCTION pgr_drivingDistance(
    ...
    OUT agg_cost FLOAT)
  RETURNS SETOF RECORD AS
     '$libdir/libpgrouting-2.6', 'driving_many_to_dist'
 LANGUAGE c VOLATILE STRICT;
...
CREATE OR REPLACE FUNCTION _pgr_ksp(edges_sql text, start_vid bigint, end_vid bigint, k integer, directed boolean, heap_paths boolean,
  ...
  RETURNS SETOF RECORD AS
    '$libdir/libpgrouting-2.6', 'kshortest_path'
...
CREATE OR REPLACE FUNCTION pgr_withPointsKSP(
  ...
  RETURNS SETOF RECORD AS
    '$libdir/libpgrouting-2.6', 'withPoints_ksp'
    LANGUAGE c STABLE STRICT;
...
AS '$libdir/libpgrouting-2.6', 'newTSP'
LANGUAGE c VOLATILE STRICT;
...
CREATE OR REPLACE FUNCTION pgr_eucledianTSP(
    ...
    OUT agg_cost FLOAT)
RETURNS SETOF record
AS '$libdir/libpgrouting-2.6', 'eucledianTSP'
LANGUAGE c VOLATILE STRICT;
...
CREATE OR REPLACE FUNCTION pgr_alphashape(sql text, alpha float8 DEFAULT 0, OUT x float8, OUT y float8)
    RETURNS SETOF record
    AS '$libdir/libpgrouting-2.6', 'alphashape'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION _pgr_bdAstar(
    ...
    OUT agg_cost FLOAT)
RETURNS SETOF RECORD AS
    '$libdir/libpgrouting-2.6', 'bd_astar'
LANGUAGE C VOLATILE;
...
CREATE OR REPLACE FUNCTION _pgr_bdDijkstra(
    ...
    OUT agg_cost FLOAT)
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'bdDijkstra'
LANGUAGE c VOLATILE;
...
AS '$libdir/libpgrouting-2.6', 'turn_restrict_shortest_path_vertex'
...
CREATE OR REPLACE FUNCTION _pgr_trsp(
    ...
    turn_restrict_sql text DEFAULT null)
RETURNS SETOF pgr_costResult
AS '$libdir/libpgrouting-2.6', 'turn_restrict_shortest_path_edge'
LANGUAGE 'c' IMMUTABLE;
...
CREATE OR REPLACE FUNCTION _pgr_maxflow(
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'max_flow_many_to_many'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_maxCardinalityMatch(
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'maximum_cardinality_matching'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_edgeDisjointPaths(
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'edge_disjoint_paths_many_to_many'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION pgr_contractGraph(
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'contractGraph'
    LANGUAGE c VOLATILE;
...
RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'pickDeliver'
LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION _pgr_pickDeliverEuclidean (
  ...
  RETURNS SETOF RECORD AS
 '$libdir/libpgrouting-2.6', 'pickDeliverEuclidean'
    LANGUAGE c VOLATILE;
...
CREATE OR REPLACE FUNCTION _pgr_withPoints(
    ...
    OUT agg_cost FLOAT)
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'withPoints'
LANGUAGE c VOLATILE;
...
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'lineGraph'
...
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'connectedComponents'
LANGUAGE c IMMUTABLE STRICT;
...
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'strongComponents'
LANGUAGE c IMMUTABLE STRICT;
...
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'biconnectedComponents'
LANGUAGE c IMMUTABLE STRICT;
...
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'articulationPoints'
LANGUAGE c IMMUTABLE STRICT;
...
RETURNS SETOF RECORD AS
'$libdir/libpgrouting-2.6', 'bridges'
LANGUAGE c IMMUTABLE STRICT;

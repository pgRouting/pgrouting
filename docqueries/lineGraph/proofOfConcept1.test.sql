...
DROP TABLE IF EXISTS edge_table1;
...
UPDATE edge_table1 SET the_geom = st_makeline(st_point(x1,y1),st_point(x2,y2));
SELECT pgr_createTopology('edge_table1',0.001);


-- this is a hand made test
UPDATE edge_table1 SET id = id * 100, source = 1000 * source, target = 1000 * target;
UPDATE edge_table1_vertices_pgr SET id = id * 1000;
...
FROM edge_table1_vertices_pgr AS v WHERE v.id = r.id;
...
c AS (SELECT id,source FROM edge_table1 WHERE id IN (SELECT edge FROM b))
...
c AS (SELECT id,target FROM edge_table1 WHERE id IN (SELECT edge FROM b))
...
-- A Dijkstra
SELECT * from pgr_dijkstra($$SELECT id, * FROM edge_table1$$,

...
/* --q3 */
SELECT pgr_nodeNetwork('edge_table', 0.001, the_geom => 'geom');
/* --q3.1 */
...
/* --q5 */
SELECT pgr_createTopology('edge_table_noded', 0.001, 'geom');
/* --q5.1 */

/* --q6 */
SELECT pgr_analyzegraph('edge_table_noded', 0.001, 'geom');
/* --q6.1 */

/* --q7 */
alter table edge_table drop column if exists old_id;
alter table edge_table add column old_id integer;
insert into edge_table (old_id, cost, reverse_cost, geom)
   (with
...
/* --q9 */
SELECT pgr_analyzegraph('edge_table', 0.001, 'geom', rows_where:='id not in (select old_id from edge_table where old_id is not null)');
/* --q9.1 */

/* --q10 */
SELECT pgr_analyzegraph('edge_table', 0.001, 'geom', rows_where:='old_id is null');
/* --q10.1 */

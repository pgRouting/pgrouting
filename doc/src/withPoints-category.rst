...
When points are also given as input to a base function.  function.
...
edges as required on the `Points SQL`_ query.
...
identifier (``edge_id``), with a ``fraction`` along that edge (from the source
to the target of the edge) and some additional information about which ``side``
of the edge the point is on, then processing from arbitrary points can be done
on fixed networks.

All this functions consider as many aspects as possible:

- Work for the base function graph kind:
...
- Arriving at the point:
  ...
  - On either side of the segment (street).
...
- The numbering of the points are handled with negative sign.
  ...
  - Interpretation of the sign on the vertices information
...
points such as restaurant, supermarket, post office, etc.
...
- Red arrows shows the ``(source, target)`` of the edge on the edge table
- Blue arrows shows the ``(target, source)`` of the edge on the edge table
...
- Point **1** located on edge ``(1, 2)`` and ``(2, 1)``
- Point **2** located on edge ``(12, 9)`` and ``(9, 12)``
- Point **3** located on edge ``(10, 11)``
- Point **4** located on edge ``(8, 7)`` and ``(7, 8)``
- Point **5** located on edge ``(3, 6)``
- Point **6** located on edges ``(2, 5)`` and ``(5, 2)``
...
- Arrival to the point ``-2`` can be achived only via vertex **9**.
...
- Arrival to the point ``-2`` can be achived only via vertex **12**.
...
- Arrival to the point ``-2`` can be achived only via vertex **9** or **12**.

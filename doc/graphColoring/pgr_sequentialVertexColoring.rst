...
``pgr_sequentialVertexColoring`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. code-block:: none
    ...
    pgr_sequentialVertexColoring(edges_sql, start_vid, end_vid)
...
.. index::
    single: sequentialVertexColoring(Complete signature)

..rubric:: Complete Signature

.. code-block:: none

    pgr_sequentialVertexColoring(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
